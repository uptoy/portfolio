...
import * as CategoryAPI from "services/apiCategory"
...
export const updateCategory = createAsyncThunk(
  "categories/update",
  async ({ _id, fields }: { _id: string; fields: { title: string } }, { rejectWithValue }) => {
    try {
      const response = await CategoryAPI.updateCategory(_id, fields)
      return response.data.data
    } catch (err: any) {
      const error: AxiosError<ValidationErrors> = err
