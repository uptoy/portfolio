...
export const productFormSchema = yup.object({
  ...
  category_id: yup.number().required('Category is a required field').min(0, 'Enter a price more than 0'),
  count_in_stock: yup.number().required('Stock is a required field').min(0, 'Enter a price more than 0'),
  description: yup.string().required('Description is a required field').min(0, 'Enter a price more than 0'),
  files: yup.array()
})

function isFileExists(file: File) {
  return !!file
}
...
function isValidFileType(file: File) {
  const maxFileSize = 3000000
  const supportedFileFormats = ['image/jpg', 'image/jpeg', 'image/gif', 'image/png', 'image/bmp', 'image/webp']
